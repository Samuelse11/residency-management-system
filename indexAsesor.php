...
// Verificar si el usuario está logueado y es de tipo Asesor
if (!isset($_SESSION['user_id']) || strtolower($_SESSION['user_type']) !== 'asesor') {
    header("Location: indexLogin.php");
    exit();
}

// Destruir la sesión al cargar la página si se solicita cerrar sesión
...
// Obtener el ID del asesor desde la URL o buscar por nombre de usuario
$asesor_id = isset($_GET['asesor_id']) ? intval($_GET['asesor_id']) : 0;

// Si no se proporcionó ID, intentar encontrarlo por el nombre del usuario
if ($asesor_id === 0) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Si no se encuentra el asesor, mostrar mensaje
        $mensaje_asesor = "No se encontró información de asesor para este usuario.";
    }
}

// Inicializar array de comentarios en la sesión si no existe
...
// Función para extraer la calificación total de un PDF
function extraerCalificacionPDF($filePath) {
    // Incluir la librería para leer PDF
    include('vendor/autoload.php'); // Asegúrate de tener instalado smalot/pdfparser
    ...
    try {
        ...
        // Buscar la calificación total en el texto
        if (preg_match('/Calificación total\s+(\d+)/', $text, $matches)) {
            return intval($matches[1]);
        }
        
        // Si no encuentra con ese patrón, probar otro
        if (preg_match('/Calificación total\s*\n\s*(\d+)/', $text, $matches)) {
            return intval($matches[1]);
        }
        
        return null; // No se encontró la calificación
    } catch (Exception $e) {
...
// Procesar la subida REAL de archivos
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo']) && isset($_POST['alumno_id']) && isset($_POST['revision_num'])) {
    ...
    // Extraer la calificación del PDF
    ...
    // Determinar qué columna actualizar según el número de revisión
    ...
    if ($stmt->execute()) {
        // Actualizar también la calificación del alumno (promedio de las 3 revisiones)
        ...
        if ($result->num_rows > 0) {
            ...
            // Actualizar calificación del alumno
...
if ($asesor_id > 0) {
    ...
    if ($result_asesor && $result_asesor->num_rows > 0) {
        ...
        // Verificar si el asesor tiene proyectos asignados
        if (!empty(trim($row_asesor['nombreProyecto']))) {
            $proyectos_asesor = explode(',', $row_asesor['nombreProyecto']);
            
            // Limpiar espacios en blanco alrededor de los nombres de proyectos
            $proyectos_asesor = array_map('trim', $proyectos_asesor);
            $proyectos_asesor = array_filter($proyectos_asesor); // Eliminar elementos vacíos
            
            if (!empty($proyectos_asesor)) {
                // Crear lista de proyectos para la consulta SQL
                $proyectos_list = "'" . implode("','", array_map([$conexion, 'real_escape_string'], $proyectos_asesor)) . "'";
                
                // Obtener anteproyectos asignados a este asesor
    ...
    <script>
        ...
        function verAnteproyecto(pdfData) {
            ...
            try {
                ...
                // Guardar los datos para posible descarga
        ...
        // Función para descargar anteproyecto
        ...
        // Event listener para descargar PDF desde el modal
        ...
        function changeRevisionTab(tabElement, alumnoId) {
            ...
            // Actualizar el valor de revisión_num
        ...
        // Filtrar al cargar la página (por si hay parámetros en la URL)
