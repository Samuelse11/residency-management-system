...
// Verificar si hay credenciales guardadas en cookies
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        if(password_verify($password, $usuario['password'])) {
            ...
            // Recordar contraseña si el checkbox está marcado
            if(isset($_POST['remember'])) {
                $cookie_expiry = time() + (30 * 24 * 60 * 60); // 30 días
...
function redirigirUsuario($tipo, $id, $nombre) {
    switch (strtolower($tipo)) {
        case 'alumno':
            // Buscar al alumno por nombre en la tabla alumnos
            $db_config = [
                'host' => 'localhost',
                'user' => 'root',
                'pass' => '12345',
            ...
            // Buscar por nombre en la tabla alumnos
            ...
            if ($result_alumno->num_rows > 0) {
                ...
                // Guardar el ID del alumno en la sesión (IMPORTANTE para indexDocumentos.php)
                $_SESSION['id_alumno'] = $alumno_id;
                
                // Verificar si ya tiene carrera registrada (esto indica que ya subió documentos)
                if (!empty($alumno['carrera']) && $alumno['carrera'] !== '') {
                    $conn->close();
                    header("Location: test.php?id=" . $alumno_id);
                    exit();
                } else {
                    // Si no tiene carrera, redirigir a subir documentos
                    $conn->close();
        ...
        case 'asesor':
            // Buscar el ID del asesor por su nombre
            $db_config = [
                'host' => 'localhost',
                'user' => 'root',
                'pass' => '12345',
            ...
            if ($result_asesor->num_rows > 0) {
                ...
                // Si no encuentra el asesor, redirigir a la página principal de asesor
                $conn->close();
...
<body> 
    ...
    <?php 
    // Mostrar mensaje de error en login si existe
