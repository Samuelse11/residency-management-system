...
// Verificar si hay un ID de alumno en la URL
...
// Obtener asesor asignado (si existe)
$asesor_asignado = "--Por definir--";
$tiene_asesor = false; // Variable para controlar si tiene asesor asignado

if (!empty($alumno_info['nombreProyecto'])) {
    // Dividir los proyectos por comas y buscar cada uno en la tabla de asesores
    ...
    if (!empty($asesores_encontrados)) {
        ...
        // Actualizar el estado del alumno si no está marcado como completado
...
<head>
    ...
    <style>
        /* Estilo para el recuadro del nombre del proyecto */
        .nombre-proyecto-box {
        ...
        /* Estilo para el área de asesores */
        .asesores-box {
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Verificar si hay asesores guardados en localStorage
            const asesoresGuardados = localStorage.getItem(`asesores_${alumnoId}`);
            if (asesoresGuardados) {
                bloquearEdicionAsesores(asesoresGuardados, false); // No marcar como completado
            }
            
            // Verificar el estado del alumno al cargar la página
            verificarEstadoAlumno();
            
            // Verificar el dictamen
            verificarDictamen();
            
            // Verificar si ya tiene asesor asignado
            verificarAsesorAsignado();
            
            // Verificar si hay asesor asignado desde PHP
            <?php if ($tiene_asesor): ?>
        ...
        function verificarEstadoAlumno() {
            // Marcar documentación previa como completada (círculo 1)
            const circulo1 = document.getElementById('circulo1');
            circulo1.innerHTML = '&#x2714;';
            circulo1.classList.add('circulo-completo');
            
            // Verificar si ya tiene un proyecto registrado
            verificarProyecto();
        }
            ...
            fetch(`obtener_asesor_asignado.php?id_alumno=${alumnoId}`)
                ...
                .then(data => {
                    if(data.asesor && data.asesor !== '--Por definir--') {
                        // Actualizar el display del asesor
        ...
        function actualizarDictamen(dictamen) {
            ...
            // Aplicar estilos según el tipo de dictamen
        ...
        function bloquearEdicionAsesores(asesores, marcarCompleto = true) {
            ...
            // Mostrar el div de visualización y ocultar el textarea
            ...
            // Solo marcar como completado si se especifica
            if (marcarCompleto) {
                const circulo4 = document.getElementById('circulo4');
                circulo4.innerHTML = '&#x2714;';
                circulo4.classList.add('circulo-completo');
        ...
        function guardarAsesores() {
            ...
            // Validar que no haya más de 3 asesores
            ...
            // Simular guardado (sin enviar a servidor)
            setTimeout(() => {
                Swal.close();
                Swal.fire({
                    icon: 'success',
                ...
                // Bloquear edición de asesores (sin marcar como completado)
        ...
        function verificarPermisoEdicion(dictamen) {
            if (!dictamen || dictamen === '--Por definir--' || 
                ...
                return; // Permite edición
            }
            
            // Si el dictamen es "Aprobado", bloquear la edición
            if (dictamen.toLowerCase().includes('aprobado') && 
                !dictamen.toLowerCase().includes('modificaciones')) {
        ...
        function subirAnteproyecto() {
            // Verificar si ya tiene un dictamen que no permita edición
            fetch(`obtener_dictamen.php?id_alumno=${alumnoId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.dictamen && 
