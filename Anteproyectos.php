...
// Procesar el formulario para agregar nuevo profesor
...
// Procesar el formulario de asignaciones
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['asesor'])) {
    ...
    if (!empty($ids_proyectos)) {
        ...
        // Agrupar proyectos y carreras por profesor
        ...
        // Actualizar las asignaciones en la base de datos
        foreach ($proyectos_por_profesor as $profesor_id => $proyectos) {
            $proyectos_str = implode(', ', $proyectos);
            
            // Obtener la carrera más común asignada a este profesor
...
// Obtener todos los anteproyectos aprobados
...
// Crear un array de profesores para fácil acceso
$lista_profesores = [];
...
// Obtener las asignaciones actuales (proyecto => profesor_id)
$asignaciones = [];
$proyectos_por_profesor = [];
